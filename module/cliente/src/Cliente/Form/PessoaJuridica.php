<?php
 ...
 class PessoaJuridica extends BaseForm {
   ...
   public function __construct($name, $serviceLocator, $campos)
    {
        ...
        //cnpj
        if($campos['cnpj']['aparecer'] == 'S'){
            $atributos = $this->booleanByAtivo($campos['cnpj']['obrigatorio']);
            $this->textInputCnpj('cnpj', $atributos['prefixo'].$campos['cnpj']['label_campo'], $atributos['obrigatorio'], 'CNPJ');
        }
        ...
        if($campos['como_ficou_sabendo']['aparecer'] == 'S'){
            $comoFicouSabendo = $this->getServiceLocator()->get('ComoFicouSabendo')->fetchAll(array('id', 'nome'))->toArray();
            $comoFicouSabendo = $serviceEstadoCivil->prepareForDropDown($comoFicouSabendo, array('id', 'nome'));
